...
open class IBANtools: NSObject {
  ...
  class var institutesInfo: [String: InstituteInfo] {
    if (_institutesInfo.count == 0) {
      ...
      if let resourcePath = bundle.path(forResource: "assets/iban/eu_all_mfi", ofType: "txt") {
  ...
  fileprivate class func loadData(_ path: String) {
    if FileManager.default.fileExists(atPath: path + "assets/iban/eu_all_mfi.txt") {
      guard let content = try? NSString(contentsOfFile: path + "assets/iban/eu_all_mfi.txt", encoding: String.Encoding.utf8.rawValue) else { /* TODO: Return an error code */ return }
      let bundle = Bundle(for: IBANtools.self);

      usedPath = path + "assets/iban/eu_all_mfi.txt";
      _institutesInfo = [:];
