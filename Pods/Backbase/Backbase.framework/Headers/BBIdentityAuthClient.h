...
#import "BBOAuth2AuthClient.h"
#import "DeviceAuthClient.h"
#import "BBIdentityAuthenticatorsProvider.h"
@protocol BBIdentityAuthClientDelegate;
@protocol FIDORegistrationDelegate;
@class BBAuthenticator;

#ifndef BBIDENTITYAUTHCLIENT_CLASS
#define BBIDENTITYAUTHCLIENT_CLASS
 ...
 */
@interface BBIdentityAuthClient : BBOAuth2AuthClient <DeviceAuthClient, BBIdentityAuthenticatorsProvider>
 ...
 */
- (BOOL)addAuthenticator:(BBAuthenticator* _Nonnull)authenticator error:(NSError* _Nullable* _Nullable)error;
...
- (void)startUAFRegistrationWithUsername:(NSString* _Nonnull)username
                                delegate:(NSObject<FIDORegistrationDelegate>* _Nonnull)delegate;
 ...
 * @param delegate Where the result of the operation will be notified to.
 */
- (void)invalidateTokensWithDelegate:(NSObject<BBIdentityAuthClientDelegate>* _Nonnull)delegate;
@end
